...
TEST(CarpParserTest, LoadInvalidFileIncorrectNumberOfNodes) {
  ...
  EXPECT_CALL(log, Log(base_logging::ERROR, testing::_,
  ...
  CarpParser parser;
  EXPECT_FALSE(parser.LoadFile(file::JoinPath(
      absl::GetFlag(FLAGS_test_srcdir), "carp_gdb19_incorrecto_vertices.dat")));
}

TEST(CarpParserTest, LoadInvalidFileIncorrectNumberOfArcsWithServicings) {
  testing::ScopedMockLog log(testing::kDoNotCaptureLogsYet);
  EXPECT_CALL(
      log, Log(base_logging::ERROR, testing::_,
  ...
  CarpParser parser;
  EXPECT_FALSE(parser.LoadFile(file::JoinPath(
      absl::GetFlag(FLAGS_test_srcdir), "carp_gdb19_incorrecto_arireq.dat")));
}
...
TEST(CarpParserTest, LoadInvalidFileIncorrectNumberOfArcsWithoutServicings) {
  ...
  CarpParser parser;
  EXPECT_FALSE(parser.LoadFile(file::JoinPath(
      absl::GetFlag(FLAGS_test_srcdir), "carp_gdb19_incorrecto_arinoreq.dat")));
}
...
TEST(CarpParserTest, LoadInvalidFileIncorrectTypeOfArcCost) {
  ...
  CarpParser parser;
  EXPECT_FALSE(parser.LoadFile(file::JoinPath(
      absl::GetFlag(FLAGS_test_srcdir), "carp_gdb19_incorrecto_tipo.dat")));
}
...
TEST(CarpParserTest, LoadInvalidFileIncorrectTotalServicingCost) {
  ...
  CarpParser parser;
  EXPECT_FALSE(parser.LoadFile(file::JoinPath(
      absl::GetFlag(FLAGS_test_srcdir), "carp_gdb19_incorrecto_coste.dat")));
}
...
TEST(CarpParserTest, LoadInvalidFileIncorrectDepot) {
  ...
  CarpParser parser;
  EXPECT_FALSE(parser.LoadFile(file::JoinPath(
      absl::GetFlag(FLAGS_test_srcdir), "carp_gdb19_incorrecto_deposito.dat")));
}
...
TEST(CarpParserTest, LoadInvalidFileServicingForArcsWithoutServicing) {
  ...
  CarpParser parser;
  EXPECT_FALSE(parser.LoadFile(file::JoinPath(
      absl::GetFlag(FLAGS_test_srcdir), "testdata/carp_gdb19_mixed_arcs.dat")));
}

TEST(CarpParserTest, LoadInvalidFileServicingForArcsInWrongOrder) {
  testing::ScopedMockLog log(testing::kDoNotCaptureLogsYet);
  EXPECT_CALL(
      log, Log(base_logging::ERROR, testing::_, "Unexpected keyword: demanda"));
  EXPECT_CALL(log, Log(base_logging::ERROR, testing::_,
